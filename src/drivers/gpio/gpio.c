/* simple gpio driver for stm32f103 (hal)
...
{
  if (!p || !p->port) { s_last_error = -100; return gpio_err; }
...
gpio_status_t gpio_setup_input_irq(const gpio_pin_t* p,
                                   ...
                                   uint32_t prio)
{
  if (!p || !p->port) { s_last_error = -105; return gpio_err; }
  ...
  switch (p->pin) {
    ...
    case GPIO_PIN_7:
    case GPIO_PIN_8:
    case GPIO_PIN_9:  irqn = EXTI9_5_IRQn;  break;
